...
//funcation
const signToken = (id,name,email,phote,role)=>{
    return jwt.sign({id:id,name:name,email:email,phote:phote,role:role}, process.env.JWT_TOkEN_KEY,{
...
exports.signup =  async(req,res)=>{
    try{
        const newUser = await User.create({
            ...
            phote: req.file ? req.file.path : null,
            role: req.body.role,
...
exports.login = async(req,res)=>{
    try{
        ...
        const token = signToken(user._id,user.name,user.email,user.phote ,user.role)
        const cookieOptions ={
            expires: new Date(Date.now() + process.env.JWT_TOKEN_ESPRIES_IN * 24 * 60 *60 * 1000),
            httpOnly:true,
            secure: req.secure || req.headers['x-forwarded-proto'] === 'https'  
...
exports.forgetPassword = async(req,res,next)=>{
    ...
    const resetUrl = `${req.protocol}://${req.get(
        'host',
    )}/api/vi/users/resetPassword/${restToken}`
